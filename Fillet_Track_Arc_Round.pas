...
// Prüft, ob ein Winkel (a) zwischen Start (s) und End (e) CCW liegt
...
Var
    ...
    RunMode                : Integer; // 1 = Track+Track, 2 = Track+Arc
    ...
    // --- Variablen für Track+Track Modus ---
    N_X, N_Y               : Double;
    ...
    // --- Variablen für Track+Arc Modus ---
    T1_X, T1_Y, T2_X, T2_Y : Double; // <--- HIER SIND DIE FEHLENDEN VARIABLEN!
...
Begin
    ...
    Begin
        ...
        Begin
            ...
            Begin
                Inc(TrackCount);
                If TrackCount = 1 Then Track1 := Obj_PCB
                Else If TrackCount = 2 Then Track2 := Obj_PCB;
            End
    ...
    Else Begin
        ShowMessage('Bitte wähle entweder genau ZWEI verbundene Tracks ODER genau EINEN Track und EINEN Arc aus!');
        Exit;
    End;

    // 3. GUI aufbauen
    Form := TForm.Create(Nil);
    If RunMode = 1 Then Form.Caption := 'Track+Track Fillet'
    Else Form.Caption := 'Track+Arc Fillet';
    ...
    EditRadius := TEdit.Create(Form); EditRadius.Parent := Form; EditRadius.Text := '1.0'; EditRadius.Left := 20; EditRadius.Top := 35; EditRadius.Width := 190;
    ...
    If RadiusVal <= 0.0 Then Begin ShowMessage('Ungültiger Radius! Muss größer als 0 sein.'); Exit; End;
    ...
    Begin
        ...
        v1x := E1_X - N_X; v1y := E1_Y - N_Y;
        v2x := E2_X - N_X; v2y := E2_Y - N_Y;
        ...
        u1x := v1x / len1; u1y := v1y / len1;
        u2x := v2x / len2; u2y := v2y / len2;
        ...
        d := RadiusVal / (Sin(alpha) / Cos(alpha));
        h := RadiusVal / Sin(alpha);

        If (d > len1) Or (d > len2) Then Begin ShowMessage('Radius zu groß! Er überlappt die Leiterbahnen.'); Exit; End;

        T1x := N_X + u1x * d; T1y := N_Y + u1y * d;
        T2x := N_X + u2x * d; T2y := N_Y + u2y * d;

        ubx := u1x + u2x; uby := u1y + u2y;
        lenb := Sqrt(ubx*ubx + uby*uby);
        ubx := ubx / lenb; uby := uby / lenb;

        Cx := N_X + ubx * h; Cy := N_Y + uby * h;

        vt1x := T1x - Cx; vt1y := T1y - Cy;
        vt2x := T2x - Cx; vt2y := T2y - Cy;
        ...
        diff := Angle2 - Angle1;
        If diff < 0 Then diff := diff + 360.0;
        ...
        ShowMessage('Erfolg! Track+Track Fillet wurde erstellt.');
    End
    ...
    Begin
        T1_X := CToMM(Track1.X1); T1_Y := CToMM(Track1.Y1);
        T2_X := CToMM(Track1.X2); T2_Y := CToMM(Track1.Y2);

        Ca_X := CToMM(TheArc.XCenter); Ca_Y := CToMM(TheArc.YCenter);
        Ra   := CToMM(TheArc.Radius);
        ...
        If Not HasSolution Then Begin ShowMessage('Radius ist zu groß oder geometrisch unmöglich!'); Exit; End;
        ...
        Try Client.SendMessage('PCB:Zoom', 'Action=Redraw' , 255, Client.CurrentView); Except End;
        ShowMessage('Erfolg! Track+Arc Fillet wurde erstellt.');
    End;
End;
